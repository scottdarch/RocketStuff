...
static void demo4(void)
{
    ...
    nrf_pwm_sequence_t const pwm1_seq1 = {
        .values.p_common = stay_off_values, .length = 2, .repeats = 149, .end_delay = 0};
    ...
    nrf_drv_pwm_complex_playback(&m_pwm1, &pwm1_seq0, &pwm1_seq1, 1, NRF_DRV_PWM_FLAG_LOOP);
    nrf_drv_pwm_complex_playback(&m_pwm2, &pwm2_seq0, &pwm2_seq1, 1, NRF_DRV_PWM_FLAG_LOOP);
}
